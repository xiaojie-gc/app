...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):  # index, source
            # Start thread to read frames-1 from video stream
            print(f'{i + 1}/{n}: {s}... ', end='')
    ...
    def update(self, i, cap):
        # Read stream `i` frames-1 in daemon thread
    ...
    def __len__(self):
        return len(self.sources)  # 1E12 frames-1 = 32 streams at 30 FPS for 30 years
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        f = []  # image files
        p = Path("datasets/labels/val2017")  # os-agnostic
        f += glob.glob(str(p / '**' / '*.*'), recursive=True)

        self.label_files = sorted([x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in ["txt"]])#[-1000:]

        sa, sb = os.sep + 'labels' + os.sep, os.sep + 'images' + os.sep  # /images/, /labels/ substrings

        self.img_files = [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.jpg' for x in self.label_files]

        # print(len(self.label_files), len(self.img_files))
        ...
        if self.rect:
            ...
            s = np.array(self.shapes, dtype=np.float64)  # wh
        ...
        self.tasks = {}
        for i in range(len(self.img_files)):
            self.tasks[self.img_files[i][-16:]] = i
    ...
    def __getitem__(self, task, img_path, img_size):
        index = self.tasks[task]  # linear, shuffled, or image_weights
        hyp = self.hyp
        ...
        if mosaic:
            ...
            img0, img, (h0, w0), (h, w) = load_image2(self, img_path, img_size)
            # Letterbox
        ...
        return img0, torch.from_numpy(img), labels_out, [self.img_files[index]], [shapes]
...
def get_img_label_pair(img_file, img_size=640, batch_size=1, rect=True, stride=32, pad=0.0):
    ...
    if rect:
        ...
        im.verify()  # PIL verify
        s = np.array([exif_size(im)], dtype=np.float64)  # image size  # wh
        ar = s[:, 1] / s[:, 0]  # aspect ratio
        irect = ar.argsort()
        shapes = s[irect]  # wh
        ar = ar[irect]
    ...
    # Letterbox
    shape = batch_shapes[0] if rect else img_size  # final letterboxed shape
...
def load_image2(self, path, img_size):
    ...
    return img0, img, (h0, w0), img.shape[:2]  # img, hw_original, hw_resized
